...
async function updatePublisher(updatedData, id) {
    const updated = await db('publisher').where('id', id).update(updatedData)
    if (updated) {
        return db('publisher').where('id', id).first();
    } else {
        throw new Error();
    }
}
...
async function updatePerson(updatedData, id) {
    const updated = await db('person').where('id', id).update(updatedData)
    if (updated) {
        return db('person').where('id', id).first();
    } else {
        throw new Error();
    }
}
...
async function deletePerson(id) {
    return await db('person').where('id', id).del();
}
...
async function updateBook(updatedData, id) {
    const updated = await db('book').where('id', id).update(updatedData)
    if (updated) {
        return db('book').where('id', id).first();
    } else {
        throw new Error();
    }
}
...
async function getLending() {
    return await db('lending as l').join('book as b', 'l.book_id', 'b.id')
...
async function addLending(newLending) {
    return await db('lending').insert(newLending, "id").then(([{
        id
    }]) => {

        return db('lending as l').join('book as b', 'l.book_id', 'b.id')
            .join('person as p', 'l.person_id', 'p.id')
            .join('publisher as pu', 'b.publisher_id', 'pu.id')
            .select(
                'b.name as bookName',
...
async function getLendingById(id){
   const lend =  await db('lending as l').join('book as b', 'l.book_id', 'b.id')
   .join('person as p', 'l.person_id', 'p.id')
   .join('publisher as pu', 'b.publisher_id', 'pu.id')
   .select(
       'l.id',
...
async function updateLending(updatedData, id) {
    await db('lending').where('id', id).update(updatedData);
    return db('lending').where('id', id).first();

}

async function deleteLending(id) {
   const  lending =await db('lending').where('id', id).del();
    if(lending){
        return lending;
    }else{
        throw new Error();
