...
router.post('/',
    body('name').notEmpty().withMessage(messages.requiredAuthor),
    body('name').isLength({
        min: 3
    }).withMessage(messages.minAuthor + " 3"), async (req, res, next) => {
...
router.put('/:id',
    ...
    body('name').isLength({
        min: 3
    }).withMessage(messages.minAuthor + " 3"), (req, res) => {
        const errors = validationResult(req)
        if (!errors.isEmpty()) {
            next([400, messages.array()])
        }
        let {
            id
        } = parseInt(req.params);
        updatedData = req.body;
        Author.updateAuthor(updatedData, id).then(updated => {
